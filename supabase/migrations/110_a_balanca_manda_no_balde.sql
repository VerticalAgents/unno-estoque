-- ============================================================
-- Migration 109 — a balança manda no balde
--
-- Três defeitos da mesma costura: o que o sistema SUPÕE que há num balde
-- contra o que a balança DIZ.
--
-- ── 1. A sobra declarada era corroída em silêncio ────────────
--
-- A 084 fazia a sobra ceder quando os potes pesavam mais do que saiu das
-- embalagens: `ceder = menor(excesso, sobra_total)`. O Lucca declarou 808 g de
-- chocolate e ficaram 86 g gravados. A regra se apoiava numa frase que está
-- errada, e ela está escrita lá com todas as letras:
--
--     "Sobrar é fisicamente impossível — o pote não pode receber mais do que
--      saiu das embalagens."
--
-- É possível. O pacote do fornecedor pode vir com mais do que a nota diz, e
-- foi o próprio Lucca quem deu o contraexemplo (400 g a mais num pacote).
-- Quando isso acontece, o conserto certo não é apagar o que foi medido: é
-- reconhecer que a embalagem tinha mais.
--
-- ── 2. O quanto entrou dependia de uma suposição ─────────────
--
-- `colocou` era `peso final do pote - o que o sistema achava que havia dentro`.
-- Se a suposição estivesse errada — e ela erra sempre que a produção consome
-- diferente do teórico — o erro entrava disfarçado de excesso.
--
-- Agora o pote é pesado ANTES e DEPOIS, e `colocou` vira subtração de duas
-- medições. A diferença entre o peso de antes e o que o sistema supunha deixa
-- de se esconder: vira ACERTO DE BALDE, com nome e movimento próprios, fora do
-- número de perda.
--
-- Isso resolve de quebra a ordem de esvaziamento dos baldes, que o teórico
-- chuta pelo nome do pote: o erro não se acumula, morre na próxima recarga
-- daquele balde. O balde que ninguém toca fica para a auditoria.
--
-- ── 3. A folga virou escolha ─────────────────────────────────
--
-- Duas balanças nunca fecham no grama, então alguma folga é inevitável. Mas
-- 2% fixos eram demais e ninguém tinha escolhido aquilo. Agora é configuração,
-- e nasce DESLIGADA: vale exato o que foi pesado.
--
-- O aviso de "faltou bipar uma embalagem" continua existindo mesmo com a folga
-- desligada, com limiar de 1% — ele não é sobre tolerância de balança, é sobre
-- um erro de operação que vale a pena pegar. Desligar a folga muda o que o
-- sistema FAZ com a diferença, não se ele avisa.
--
-- Partiu de `pg_get_functiondef` — ver CLAUDE.md.
-- ============================================================

-- ── A configuração ──────────────────────────────────────────
-- NULL = desligada, e é o padrão. NULL não é zero aqui: os dois se comportam
-- igual no cálculo, mas só um deles diz "não use folga" em vez de "tolere
-- nada".
ALTER TABLE configuracoes_sistema
  ADD COLUMN IF NOT EXISTS folga_balanca_pct NUMERIC(5,2);

COMMENT ON COLUMN configuracoes_sistema.folga_balanca_pct IS
  'Percentual de diferença tratado como ruído de balança no abastecimento. '
  'NULL = desligada: vale exato o que foi pesado, e toda diferença vira '
  'registro. Ligada, diferenças menores que isto fazem a sobra ceder.';

-- ============================================================
-- registrar_abastecimento — pesa antes e depois
--
-- A assinatura antiga sai de cena: com as duas no catálogo, uma chamada com o
-- formato antigo de `p_potes` casaria com ela e o novo nunca chegaria. É a
-- lição da 042, repetida na 102.
-- ============================================================
DROP FUNCTION IF EXISTS registrar_abastecimento(UUID, UUID, UUID, JSONB, JSONB, TEXT);

CREATE OR REPLACE FUNCTION registrar_abastecimento(
  p_empresa_id     UUID,
  p_responsavel_id UUID,
  p_insumo_id      UUID,
  p_potes          JSONB,   -- [{local_id, antes, depois, medido}] na unidade do cadastro
...
COMMENT ON FUNCTION registrar_abastecimento(UUID, UUID, UUID, JSONB, JSONB, TEXT) IS
  'Registra o abastecimento dos recipientes. O pote é pesado ANTES e DEPOIS, '
  'então o que entrou é subtração de duas medições e não depende do saldo que o '
  'sistema supunha. A diferença entre a pesagem de antes e esse saldo vira '
  'acerto de recipiente, fora do número de perda.';
