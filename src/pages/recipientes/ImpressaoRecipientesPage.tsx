...
export function ImpressaoRecipientesPage() {
  ...
  // Quando a tela de etiquetas pendentes manda para cá, ela diz QUAIS. Sem
  // isso a lista viria com os 51 recipientes marcados e quem quer imprimir
  // os 6 novos teria de desmarcar 45.
  const preSelecionados = (useLocation().state as { recipienteIds?: string[] } | null)?.recipienteIds
    ...
    supabase
      ...
      .then(({ data }) => {
        ...
        setSelecionados(new Set(
          preSelecionados?.length
            ? lista.filter(r => preSelecionados.includes(r.id)).map(r => r.id)
            : lista.filter(r => r.ativo).map(r => r.id)))
        setCarregando(false)
      })
  }, [profile])
    ...
    <>
      ...
      <div className="etiqueta-screen-ui p-4 sm:p-6 max-w-3xl mx-auto">
        ...
        <Button
          ...
          onClick={async () => {
            // No clique, e nao na saida do papel — o navegador nao tem como
            // saber se imprimiu. Mesmo limite da tela de lotes; reimprimir
            // continua possivel (migration 120).
            const ids = [...new Set(etiquetas.map(e => e.recipiente.id))]
            await supabase.from('locais').update({ etiqueta_impressa: true }).in('id', ids)
            window.print()
          }}
          icon={
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
